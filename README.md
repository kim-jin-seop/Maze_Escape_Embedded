...
(1) 좌 우측에 벽이 모두 없는 경우 방위에 따라 움직일 방향을 정해줍니다. 방위가 동쪽이면 좌측으로 그 외에 경우는 모두 우측으로 설정합니다.   (2) 좌 우측에 벽이 하나 혹은 2개 다 있는 경우 오른쪽에 벽이 있다면, 오른쪽 벽을 피해 왼쪽으로 그 외에 경우 모두 오른쪽으로 설정하여줍니다.  - 앞에 물체가 없는 경우   
